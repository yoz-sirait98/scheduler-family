  ...
  <div class="space-y-5 max-w-4xl mx-auto">
    ...
    <div class="flex items-center justify-between gap-3 pb-2 border-b border-slate-200/80 dark:border-slate-800/80">
      <div>
        ...
        <p class="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">
          Organize family and personal activities with custom colors & icons
        </p>
      </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2.5">
      <div
        ...
        :key="cat.id"
        class="p-3.5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 shadow-2xs flex items-center justify-between gap-3 group hover:border-indigo-300 dark:hover:border-indigo-800 transition-all"
      >
        <div class="flex items-center gap-2.5 min-w-0">
          <div
            class="w-9 h-9 rounded-xl flex items-center justify-center shadow-xs shrink-0"
            :style="{
    ...
    <!-- Category Modal -->
    <div v-if="isModalOpen" class="fixed inset-0 z-50 overflow-y-auto bg-slate-950/70 backdrop-blur-xs flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div class="fixed inset-0" @click="isModalOpen = false"></div>
      <div class="relative w-full sm:max-w-sm bg-white dark:bg-slate-900 rounded-t-3xl sm:rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 p-5 sm:p-6 z-10 animate-in fade-in slide-in-from-bottom-6 duration-200 pb-[env(safe-area-inset-bottom,20px)]">
        ...
        <form @submit.prevent="saveCategory" class="space-y-4">
          <div>
            ...
            <input
              ...
              class="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-500"
            />
          </div>
          ...
          <div>
            ...
            <div class="grid grid-cols-6 gap-1.5 max-h-36 overflow-y-auto p-1 border rounded-xl border-slate-100 dark:border-slate-800">
              <button
              ...
              >
                <CategoryIcon :name="iconName" size="sm" />
              </button>
            </div>
          </div>
          ...
          <div class="flex items-center gap-3 pt-2">
            ...
            <button
              type="submit"
              class="flex-1 py-2.5 px-4 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold shadow-xs active:scale-95"
            >
              Save
            </button>
          </div>
